...
#include "testcli1CLP.h"

// Usign local nifti to read images
#include <nifti1_io.h>
...
/**
* CLI test #1: check that image CLP arguments are passed as expected to 
...
int main (int argc, char *argv[])
{
    ...
    if( !image )
    {
        std::cerr << "Test #1 FAILED: could not load the input image." << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if( !refImage )
    {
        std::cerr << "Test #1 FAILED: could not load the reference input image." << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if(!res)
    {
        std::cerr << "Test #1 FAILED: images were not passed correctly." << std::endl;
        return EXIT_FAILURE;
    }

    // SUCCESS
    std::cout << "Test #1 PASSED: images were passed correctly." << std::endl;
    return EXIT_SUCCESS;
}
